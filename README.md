...
`POST /video-download` is the single endpoint used by the iPhone Shortcut. It immediately returns `success`, a random `job_id`, `status: "queued"`, a `poll_url`, and `files: []`; the same `/video-download/{job_id}` path is polled until `status` is `completed` or `failed`, and completed files are served from `/video-download/{job_id}/{filename}`. Creating a job remains protected by the Shortcut's embedded `X-API-Key`; its unguessable, temporary poll URL works without re-entering or exposing that key. The Shortcut keeps each poll response as a named dictionary before checking `files` or `error`, so an absent pending-error field cannot abort the repeat before media is ready. The Share Sheet URL remains the only user input. This prevents Shortcuts from timing out while yt-dlp or FFmpeg work. Completed file URLs remain valid for one hour; queued, active, and failed jobs remain isolated in their own job directory.

...
`POST /v1/shortcut-downloads` has the same request shape and is intended for the iPhone Shortcut. It immediately returns `success`, a random `job_id`, `status: "queued"`, a `poll_url`, and `files: []`. The Shortcut must repeat a `GET` to that opaque `poll_url` until `status` is `completed` or `failed`; completed responses replace the empty array with downloaded media, while failed responses include a readable `error` string. Creating a job remains protected by the Shortcut's embedded `X-API-Key`; its unguessable, temporary `poll_url` works without re-entering or exposing that key. The Shortcut keeps each poll response as a named dictionary before checking `files` or `error`, so an absent pending-error field cannot abort the repeat before media is ready. The Share Sheet URL remains the only user input. This prevents Shortcuts from timing out while yt-dlp or FFmpeg work. Completed file URLs remain valid for one hour; queued, active, and failed jobs remain isolated in their own job directory.

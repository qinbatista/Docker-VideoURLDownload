...
`POST /v1/shortcut-downloads` has the same request shape and is intended for the iPhone Shortcut. It immediately returns `success`, a random `job_id`, `status: "queued"`, and a `poll_url`. The Shortcut must repeat a `GET` to that opaque `poll_url` until `status` is `completed` or `failed`; only `completed` responses contain `files`. Creating a job remains protected by the Shortcut's embedded `X-API-Key`; its unguessable, temporary `poll_url` works without re-entering or exposing that key. Queued and downloading responses intentionally omit `files`, so they cannot be interpreted as a completed job with no media while the Shortcut update is syncing to the phone. The Share Sheet URL remains the only user input. This prevents Shortcuts from timing out while yt-dlp or FFmpeg work. Completed file URLs remain valid for one hour; queued, active, and failed jobs remain isolated in their own job directory.
